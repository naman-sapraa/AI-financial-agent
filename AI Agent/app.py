...
class IndianFinancialMarketAgent:
    def __init__(self, use_historical_data=False):
        ...
        logger.info("Indian Financial Market Agent initialized (Historical Mode: {})".format(self.use_historical_data))
    ...
    def initialize_database(self):
        ...
        os.makedirs('data/historical_prices', exist_ok=True)
        
        logger.info("Database initialized")
    
    def load_historical_prices(self):
        """Load historical market data from CSV files stored in data/historical_prices"""
        logger.info("Loading historical market data from local CSV files...")
        # Load data for individual stocks
        for stock in self.stocks:
            file_path = os.path.join('data', 'historical_prices', f"{stock.replace('.NS','')}.csv")
            if os.path.exists(file_path):
                try:
                    df = pd.read_csv(file_path, parse_dates=['Date'], index_col='Date')
                    self.historical_prices[stock] = df.sort_index()
                    # Use the last two rows to compute current, previous, change, and volume
                    if len(df) >= 2:
                        last_two = df.tail(2)
                        current = last_two['Close'].iloc[-1]
                        previous = last_two['Close'].iloc[-2]
                        change = ((current - previous) / previous) * 100
                        volume = last_two['Volume'].iloc[-1]
                        ...
                        logger.info(f"Loaded historical data for {stock}: {current:.2f}")
                except Exception as e:
                    logger.error(f"Error loading historical data for {stock}: {str(e)}")
            else:
                logger.warning(f"Historical file not found for {stock}: {file_path}")
        
        # Load data for indices (if available as CSV)
        for index_name, index_symbol in self.indices.items():
            file_path = os.path.join('data', 'historical_prices', f"{index_name.replace(' ', '_')}.csv")
            if os.path.exists(file_path):
                try:
                    df = pd.read_csv(file_path, parse_dates=['Date'], index_col='Date')
                    df = df.sort_index()
                    if len(df) >= 2:
                        ...
                        previous = last_two['Close'].iloc[-2]
                        change = ((current - previous) / previous) * 100
                        volume = last_two['Volume'].iloc[-1] if 'Volume' in last_two.columns else None
                        ...
                        logger.info(f"Loaded historical data for {index_name}: {current:.2f}")
                except Exception as e:
                    logger.error(f"Error loading historical data for {index_name}: {str(e)}")
            else:
                logger.warning(f"Historical file not found for {index_name}: {file_path}")
    
    def fetch_market_data(self):
        """Fetch market data for indices and stocks. Uses historical CSVs if use_historical_data=True."""
        if self.use_historical_data:
            self.load_historical_prices()
            logger.info("Market data loaded from historical files")
    ...
    def fetch_news_data(self):
        ...
        # (News fetching code remains unchanged)
        sources = [
    ...
    def fetch_economic_indicators(self):
        ...
        self.economic_indicators = {
            ...
            'Repo_Rate': 6.5,
            'Unemployment': 7.8,
            'Foreign_Exchange_Reserves': 642.5,
            'INR_USD': 82.94,
    ...
    def perform_technical_analysis(self):
        """Perform technical analysis on stock price data using historical data"""
        ...
        if overall_trend == 'bullish':
            ...
            if len(overbought_stocks) > len(self.analysis_results.get('technical', {})) / 3:
                outlook['vulnerabilities'].append({
                    ...
                    'affected_stocks': overbought_stocks[:5]
                })
    ...
    def assess_data_confidence(self):
        ...
        if self.news_data:
            ...
            news_confidence = min(1.0, len(self.news_data) / 20) * 0.5
            news_confidence += min(1.0, recent_news / 5) * 0.5
    ...
    def _generate_detailed_analysis(self):
        ...
        for stock1, stock2, corr in high_corr[:3]:
        ...
        for stock1, stock2, corr in inverse_corr[:3]:
            stock1_name = stock1.split('.')[0]
            stock2_name = stock2.split('.')[0]
...
if __name__ == "__main__":
    try:
        # Set use_historical_data to True for past-data based analysis
        agent = IndianFinancialMarketAgent(use_historical_data=True)
